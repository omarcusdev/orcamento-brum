...
const WhatsappStatusEntregaPanel = ({ initial, expanded, onToggleExpand }: Props) => {
  const { on: master, toggle, error, setError } = useOptimisticFlag(
    initial.master,
    (next) => setWhatsappStatusFlag("master", next),
  )
  ...
  return (
    <FeaturePanel
      icon={Truck}
      title="Avisar status de entrega"
      description="Manda mensagem automática quando o pedido muda de status. Escolha em quais status e edite cada texto."
      on={master}
      onToggle={(next) => { setSalvo(null); toggle(next) }}
      switchId="whatsapp_status_entrega_ativo"
      error={error}
      collapseLabel="Mensagens por status"
      collapseHint={`(${STATUS_NOTIFY_STATUSES.filter((s) => porStatus[s].ativo).length} de ${STATUS_NOTIFY_STATUSES.length} ligados)`}
      expanded={expanded}
